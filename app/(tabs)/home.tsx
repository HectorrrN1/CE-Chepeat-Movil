  ...
  const fetchProductsBySellerId = async (sellerId: string) => {
    try {
      ...
      if (response.status === 200) {
        // Ordenar los productos por la fecha 'updatedAt' en orden descendente
        const productosOrdenados = response.data.sort((a: any, b: any) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime());
